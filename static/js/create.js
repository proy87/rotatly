...
(function() {
  var board_palette_dom, color_palette_dom, copy_outline, dom, game_dom, icon_class, init_handlers, insert_icon, item_click_handler, k, key, len, outline_dom, outline_hidden_dom, outlines_dom, palette_dom, ref, remove_data, selected, selected_outline, set_data;
  ...
  copy_outline = function(src, target, erase_data = false) {
  ...
  outlines_dom.forEach(function(outline) {
    return outline.addEventListener('click', function() {
      ...
      return copy_outline(selected_outline, game_dom);
    });
  });
  ...
  insert_icon = function(color_class, index, name, palette, icon_class, key) {
    var icon, idx, inserted, k, len, new_icon, ref;
    ...
    new_icon.addEventListener('click', function() {
      return item_click_handler.call(this, key);
    });
    inserted = false;
    ref = palette.getElementsByClassName(icon_class);
    ...
    if (!inserted) {
      return palette.appendChild(new_icon);
    }
  };

  remove_data = function(cell, palette, icon_class, key) {
    var color_class, span;
    color_class = cell.getAttribute('data-class');
    if (color_class) {
      cell.classList.remove(color_class);
      insert_icon(cell.getAttribute('data-class'), cell.getAttribute('data-index'), cell.getAttribute('data-name'), palette, icon_class, key);
    }
  ...
  set_data = function(cell, klass, index, name) {
    ...
    if (name) {
      return cell.querySelector('span').innerHTML = name;
    }
  };

  init_handlers = function(key, icon_class, dom, palette_dom) {
    var item, k, len, ref;
    ref = palette_dom.getElementsByClassName(icon_class);
    for (k = 0, len = ref.length; k < len; k++) {
      item = ref[k];
    ...
    return dom.querySelectorAll('td').forEach(function(cell) {
      return cell.addEventListener("click", function() {
        var cell_color_class, index, klass, name, selected_value, value;
        value = cell.getAttribute('data-value');
        selected_value = selected[key];
        if (selected_value) {
          cell_color_class = cell.getAttribute('data-class');
          if (cell_color_class) {
            insert_icon(cell_color_class, cell.getAttribute('data-index'), cell.getAttribute('data-name'), palette_dom, icon_class, key);
          }
          ...
          if (key === 'color_icon') {
            dom.querySelectorAll(`td[data-value='${value}']`).forEach(function(c) {
          ...
          if (cell_color_class) {
            if (key === 'color_icon') {
              return dom.querySelectorAll(`td[data-value='${value}']`).forEach(function(c) {
                return remove_data(c, palette_dom, icon_class, key);
              });
            } else {
              return remove_data(cell, palette_dom, icon_class, key);
            }
          }
        }
      });
  ...
  ref = [['color_icon', 'color-icon', outline_dom, color_palette_dom], ['board_item', 'board-item', game_dom, board_palette_dom]];
  for (k = 0, len = ref.length; k < len; k++) {
    [key, icon_class, dom, palette_dom] = ref[k];
