...
(function() {
  var active_item_clone, board_palette_dom, color_palette_dom, copy_outline, create_clone, dom, game_dom, i, icon_class, init_handlers, item_click_handler, k, key, len, outline_dom, outline_hidden_dom, outline_table, outlines_dom, palette_dom, ref, remove_data, selected, selected_outline, set_data, snap_targets;
    ...
    if (!t.parentNode.classList.contains('tet-O')) {
      ...
      return t.addEventListener('click', function() {
        if (t.dataset.isDragging === 'true') {
          return t.dataset.isDragging = false;
        } else {
          t.dataset.rotationAngle = parseInt(t.dataset.rotationAngle) + 90;
          return t.style.setProperty('--rotation', `${t.dataset.rotationAngle}deg`);
        }
      });
    }
  });
  ...
  snap_targets = function(index) {
    ...
    inner = function(x, y, interaction) {
      ...
      points = [];
      for (i = l = 0, ref1 = outline_table.rows.length - 1; (0 <= ref1 ? l < ref1 : l > ref1); i = 0 <= ref1 ? ++l : --l) {
        for (j = m = 0, ref2 = outline_table.rows[0].cells.length - 1; (0 <= ref2 ? m < ref2 : m > ref2); j = 0 <= ref2 ? ++m : --m) {
          points.push({
  ...
  interact('.tetramino').draggable({
    onstart: function(evt) {
      ...
      return active_item_clone = create_clone(el);
    },
    onmove: function(evt) {
      var el, x, y;
      el = evt.target;
